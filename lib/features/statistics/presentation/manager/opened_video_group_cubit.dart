...
class OpenedVideoGroupCubit extends Cubit<CubitStates> {
  OpenedVideoGroupCubit(this.useCase) : super(InitialState());
  ParentStatisticsForChildrenUseCase useCase;

  getStudentOpenGroupVideosRate({required int studentId}) {
    managerExecute<dynamic>(useCase.getStudentOpenGroupVideosRate(studentId: studentId),
        onStart: () {
          emit(LoadingState());
        },
        onSuccess: (data) {
